...
def get_who_data(url, hrp_countries, gho_countries, region):
    ...
    dict_regions = pd.DataFrame(
        region.iso3_to_region.items(), columns=["ISO3", "Regional_office"]
    )
    df = pd.merge(
        left=df, right=dict_regions, left_on="ISO_3_CODE", right_on="ISO3", how="left"
    )
...
def get_who_covid(
    ...
    if (
        scrapers
        and not any(scraper in name for scraper in scrapers)
        and not any(scraper in outputs["gsheets"].updatetabs for scraper in scrapers)
    ):
    ...
    # get WHO data
    source_date, df_world, df_gho, df_series, df_WHO = get_who_data(
        datasetinfo["url"], hrp_countries, gho_countries, region
    )
    df_pop = pd.DataFrame.from_records(
        list(population_lookup.items()), columns=["Country Code", "population"]
    )
    ...
    for rows in json_df:
        ...
        outputs["json"].add_data_rows_by_key(
    ...
    sources = [
        (hxltag, source_date, dssource, dssourceurl) for hxltag in sorted(hxltags)
    ]

    headers = [
        ["Cumulative_cases", "Cumulative_deaths"],
        ["#affected+infected", "#affected+killed"],
    ]
    ...
    sources.extend(
        [(hxltag, source_date, dssource, dssourceurl) for hxltag in series_hxltags]
    )
    return (
        headers,
        [global_cases, global_deaths],
        [gho_cases, gho_deaths],
        national_headers,
        national_columns,
        sources,
    )
