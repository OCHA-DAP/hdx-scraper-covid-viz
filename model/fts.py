        ...
        if objectsbreakdown:
            for fundobj in objectsbreakdown:
                ...
                if fund_id and fund_id == plan_id:
                    logger.info('%s: Funding=%d' % (plan_name, fund))
                    return fund
    return None
...
def get_fts(basic_auths, configuration, today, today_str, countryiso3s, scrapers=None):
    ...
    with Download(basic_auth=basic_auths.get('fts'), rate_limit={'calls': 1, 'period': 1}) as downloader:
        ...
        for plan in plans:
            ...
            if len(countryid_iso3mapping) == 1:
                ...
                if plan_type == 'humanitarian response plan':
                    ...
                    add_other_requirements_and_funding(countryiso, plan_name, allreq, allfund, allpct)
            else:
                ...
                for countryiso in allreqs:
                    if countryiso in allfunds:
                        continue
                    add_other_requirements_and_funding(countryiso, plan_name, allreqs[countryiso], None, None)
        ...
        hxltags = ghxltags + ['#value+covid+funding+hrp+total+usd', '#value+funding+other+plan_name', '#value+funding+other+required+usd', '#value+funding+other+total+usd', '#value+funding+other+pct']
        total_allreq = {'global': total_allreq}
        total_allfund = {'global': total_allfund}
        total_allpercent = {'global': total_allpercent}
        return [['RequiredFunding', 'Funding', 'PercentFunded'], ghxltags], \
               [total_allreq, total_allfund, total_allpercent], \
               [(hxltag, today_str, 'OCHA', fts_configuration['source_url']) for hxltag in ghxltags], \
               [['RequiredHRPFunding', 'HRPFunding', 'HRPPercentFunded', 'HRPCovidFunding',
                 'OtherPlans', 'RequiredOtherPlansFunding', 'OtherPlansFunding', 'OtherPlansPercentFunded'],
                 hxltags], \
               [hrp_requirements, hrp_funding, hrp_percentage, hrp_covid_funding,
